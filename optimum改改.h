...
void movewhichwhere(optimum::name n, optimum::dir d)
{
	...
	if		(d == optimum::dir::上)		role[i]->move(i, role[i]->x, (role[i]->y - 1));
	else if (d == optimum::dir::下)		role[i]->move(i, role[i]->x, (role[i]->y + 1));
	else if (d == optimum::dir::左)		role[i]->move(i, (role[i]->x - 1), role[i]->y);
	else					/*右*/		role[i]->move(i, (role[i]->x + 1), role[i]->y);
